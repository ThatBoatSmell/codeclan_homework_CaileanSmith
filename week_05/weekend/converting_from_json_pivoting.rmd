...
cards_wide %>% 
  ...
  group_by(ocg_release_year) %>% 
      mutate(avg_stat_total_by_year = floor(mean(stat_total))) %>% 
      ggplot(aes(x = ocg_release_year, y = avg_stat_total_by_year)) +
...
```
Link monsters do not have DEf values and are listed as NA - need to convert these to 0 or only add atk for stat total on links
...
hittr
for mucking about with jsons - might let me skip the wrangling step